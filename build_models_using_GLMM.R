...
null_form_early <- paste0('grazing_secs ~ dFence + dTank + grz_lag_avg + ',
                          'offset(log(grazing_wkly_sum)) + ',
                          '(1|Pasture) + (1|week)')
veg_form <- ' + Biomass + I(Biomass^2) + CP + CP:Biomass + CP:I(Biomass^2)'
pc_form <- ' + PC_dmt * PC_div'

sngl_lag <- TRUE

#### Build all models ####
for(yr_i in c(2016, 2017, 2018)){
  for(seas_i in c('early', 'mid', 'late')){
    print(paste('-----', yr_i, '|', seas_i, '-----'))
    pb = txtProgressBar(min = 0, max = 6, initial = 0) 
    stepi=0
    setTxtProgressBar(pb,stepi)
    if(seas_i == 'early'){
      dropcols <- c("pred_zprob", "pred_response",
                    "grz_lag_early", "grz_lag_mid")
      null_form <- null_form_early
    } else if(seas_i == 'mid'){
      dropcols <- c("pred_zprob", "pred_response",
                    "grz_lag_mid")
      null_form <- paste0(null_form_early, ' + grz_lag_early')
    } else if (seas_i == 'late'){
      dropcols <- c("pred_zprob", "pred_response")
      null_form <- paste0(null_form_early, ' + grz_lag_early + grz_lag_mid')
    }
    ...
    df_sub <- df %>% filter(Year == yr_i & season_str == seas_i) %>%
      mutate(ID = paste(UTM_X, UTM_Y, sep="_")) %>%
      dplyr::select(-any_of(dropcols)) %>%
      na.omit(.)
    
    mod0_nbzi <- glmmTMB(formula = as.formula(null_form),
                         zi=~.,
                         family=truncated_nbinom2,
                         data=df_sub,
                         control = glmmTMBControl(parallel = 10))
    if(test_zi == TRUE){
      mod0_nb <- glmmTMB(formula = as.formula(null_form),
                         zi=~0,
                         family=nbinom2,
                         data=df_sub,
                         control = glmmTMBControl(parallel = 10))
    ...
    modVEG_nbzi <- glmmTMB(formula = as.formula(paste0(null_form, 
                                                       veg_form)),
                           zi=~.,
                           family=truncated_nbinom2,
                           data=df_sub,
    ...
    modPC_nbzi <- glmmTMB(formula = as.formula(paste0(null_form, 
                                                      pc_form)),
                          zi=~.,
                          family=truncated_nbinom2,
                          data=df_sub,
    ...
    modFULL_nbzi <- glmmTMB(formula = as.formula(paste0(null_form, 
                                                        veg_form,
                                                        pc_form)),
                            zi=~.,
                            family=truncated_nbinom2,
    ...
    if('without' %in% str_split(modFULL_nbzi$fit$message, ' ')[[1]]){
      print('Failed to converge Full model. Refitting without Biomass^2')
      full_form <- paste0(null_form, 
                          ' + Biomass * CP',
                          pc_form)
      modFULL_nbzi <- update(modFULL_nbzi, formula=as.formula(full_form))
      
      if('without' %in% str_split(modFULL_nbzi$fit$message, ' ')[[1]]){
        print('Failed to converge without Biomass^2. Refitting without interactions')
        full_form <- paste0(null_form, 
                            ' + Biomass + CP',
                            ' + PC_dmt + PC_div')
        modFULL_nbzi <- update(modFULL_nbzi, formula=as.formula(full_form))
    ...
    modFULL_TPC_nbzi <- glmmTMB(formula = as.formula(paste0(full_form, 
                                                            ' + TPC_c')),
                                zi=~.,
                                family=truncated_nbinom2,
                                data=df_sub,
    ...
    all_aic[is.na(all_aic)] <- Inf
    
    mod_names <- c('Null', 'Veg', 'PC', 'PC+Veg', 'PC+Veg+TPC')
    
    df_aic <- rbind(df_aic, 
                    data.frame(Year=yr_i,
                               season=seas_i,
                               models=mod_names,
                               AIC=all_aic,
    ...
    for(i in seq(1:5)){
      ...
      mod_test <- mods[[i]]
      if('I(Biomass^2)' %in% str_split(deparse1(mod_test$call), ' ')[[1]]){
        bm_sq <- 'True'} else {bm_sq <- 'False'}
      if('CP:Biomass' %in% str_split(deparse1(mod_test$call), ' ')[[1]]){
        ixns <- 'True'} else {ixns <- 'False'}
      if('Biomass:CP' %in% str_split(deparse1(mod_test$call), ' ')[[1]]){
        ixns <- 'True'} else {ixns <- 'False'}
      if('without' %in% str_split(modFULL_nbzi$fit$message, ' ')[[1]]){
        full_conv <- 'False'} else {full_conv <- 'True'}
      df_tmp_cor <- df_sub %>% mutate(grz_lag_avg = 1.0,
                                      grz_lag_early = 1.0,
                                      grz_lag_mid = 1.0,
        ...
        mutate(pred_rel_bins = cut(pred_rel2,
                                   ...
                                   labels=seq(1, 9))) %>%
        ungroup() %>%
        summarise(cor_bin = cor(as.numeric(pred_rel_bins), grazing_rel_freq),
                  pval_bin = cor.test(as.numeric(pred_rel_bins), grazing_rel_freq)[[3]],
                  cor = cor(as.numeric(pred_rel), grazing_rel_freq),
                  pval = cor.test(as.numeric(pred_rel), grazing_rel_freq)[[3]])
      df_tmp_cor['models'] = mod_name
      df_tmp_cor['Year'] = yr_i
      df_tmp_cor['season'] = seas_i
      df_tmp_cor['bm_sq'] = bm_sq
      df_tmp_cor['interactions'] = ixns
      df_tmp_cor['full_conv'] = full_conv
      df_sub_cor <- rbind(df_sub_cor, df_tmp_cor)
    }
    
    assign(paste('modFNL', yr_i, seas_i, sep='_'),
           mods[[min(which(df_aic[df_aic$Year == yr_i & 
                                    df_aic$season == seas_i,
                                  'AICd'] < 2.0))]])
    stepi = stepi + 1
    setTxtProgressBar(pb,stepi)
    close(pb)
...
mod0_nbzi <- glmmTMB(formula = as.formula(null_form_early),
                     zi=~.,
                     family=truncated_nbinom2,
                     data=df_sub,
                     control = glmmTMBControl(parallel = 10))
if(test_zi == TRUE){
  mod0_nb <- glmmTMB(formula = as.formula(null_form_early),
                     zi=~0,
                     family=nbinom2,
                     data=df_sub,
                     control = glmmTMBControl(parallel = 10))
...
modVEG_nbzi <- glmmTMB(formula = as.formula(paste0(null_form_early, 
                                                   veg_form)),
                       zi=~.,
                       family=truncated_nbinom2,
                       data=df_sub,
...
modPC_nbzi <- glmmTMB(formula = as.formula(paste0(null_form_early, 
                                                  pc_form)),
                      zi=~.,
                      family=truncated_nbinom2,
                      data=df_sub,
...
modFULL_nbzi <- glmmTMB(formula = as.formula(paste0(null_form_early, 
                                                    veg_form,
                                                    pc_form)),
                        zi=~.,
                        family=truncated_nbinom2,
...
if('without' %in% str_split(modFULL_nbzi$fit$message, ' ')[[1]]){
  print('Failed to converge Full model. Refitting without Biomass^2')
  full_form <- paste0(null_form_early, 
                      ' + Biomass * CP',
                      pc_form)
  modFULL_nbzi <- update(modFULL_nbzi, formula=as.formula(full_form))
} else {
...
modVEG_nbzi <- glmmTMB(formula = grazing_secs ~ dFence + dTank + 
                         ...
                         Biomass + I(Biomass^2) + CP + CP:Biomass + CP:I(Biomass^2) +
                         offset(log(grazing_wkly_sum)) +
                         (1|Pasture) + (1|week),
                       zi=~.,
                       family=truncated_nbinom2,
...
df_pred %>%
  ...
  scale_fill_continuous(type = "viridis", 
                        ...
                        oob = scales::squish) +
  theme_bw() +
  geom_smooth(aes(x=as.numeric(pred_rel),
                  y=grazing_rel_freq),
              method=lm,
              color='red') +
...
pb = txtProgressBar(min = 0, max = 9, initial = 0) 
stepi=0
zi_coef_all <- data.frame()
for(yr in seq(2016, 2018, 1)){
  for(seas in unique(df$season_str)){
    setTxtProgressBar(pb,stepi)
    mod_tmp <- get(paste('modFNL', yr, seas, sep='_'))
    df_tmp <- model_parameters(mod_tmp, exponentiate=TRUE) %>%
      mutate(Parameter = case_when(Parameter == 'TPC_cOther' ~ 'TPC_cComplex',
                       TRUE ~ Parameter)) %>%
      filter(Effects == 'fixed' & Component == 'zero_inflated') %>%
      filter(str_detect(Parameter, ':', negate=TRUE),
             str_detect(Parameter, 'I\\(Biomass\\^2\\)', negate=TRUE),
             str_detect(Parameter, '(Intercept)', negate=TRUE)) %>%
      mutate(Year = yr,
             season = seas,
             Parameter = sub('avg', seas, 
                         sub('grz_', '', 
                             sub('PC_dmt', '', 
                                 sub('TPC_c', '', Parameter))))) %>%
      dplyr::select(Year, season, Parameter, Coefficient, CI_low, CI_high)
    zi_coef_all <- rbind(zi_coef_all, df_tmp)
    stepi = stepi + 1
  }
}
close(pb)
             ...
             Parameter = sub('avg', seas, 
                             sub('grz_', '', 
                                 sub('PC_dmt', '', 
                                     sub('TPC_c', '', Parameter))))) %>%
      dplyr::select(Year, season, Parameter, Coefficient, CI_low, CI_high)
    coef_all <- rbind(coef_all, df_tmp)
    stepi = stepi + 1
  }
}
close(pb)

lag_classes <- c('lag_early', 'lag_mid', 'lag_late')
geo_classes <- c('dFence', 'dTank')
veg_classes <- c('Biomass', 'CP')
pc_classes <- c('C3', 'C4', 'Saltgrass', 'Other', 'PC_div')
tpc_classes <- c('Open Slopes', 'Lowlands', 'Highlands', 'Complex')

zi_coef_all <- zi_coef_all %>%
  mutate(season = factor(season, levels=c('early', 'mid', 'late')),
         group = case_when(Parameter %in% lag_classes ~ 'Lag',
                           Parameter %in% geo_classes ~ 'Geometry',
                           Parameter %in% veg_classes ~ 'Veg conditions',
                           Parameter %in% pc_classes ~ 'Plant community',
                           Parameter %in% tpc_classes ~ 'Topography')) %>%
  mutate(group = factor(group, levels=c('Lag', 'Geometry', 
                                        'Veg conditions', 'Plant community', 
                                        'Topography'))) %>%
  arrange(group) %>%
  mutate(Parameter = factor(Parameter, levels=c(lag_classes,
                                                geo_classes,
                                                veg_classes,
                                                pc_classes,
                                                tpc_classes)))
...
ggplot(data=zi_coef_all, aes(y=Parameter, x=Coefficient, color=group)) +
  geom_point(size=2) +
  scale_y_discrete(limits=rev) +
  geom_errorbar(aes(xmin=CI_low, xmax=CI_high), width=.5) +
  facet_grid(Year~season) +
  geom_vline(xintercept = 1.0, linetype="dashed") +
  theme_bw() +
  ggtitle('(Certain zero)')

ggplot(data=coef_all, aes(y=Parameter, x=Coefficient, color=group)) +
  geom_point(size=2) +
  scale_y_discrete(limits=rev) +
  geom_errorbar(aes(xmin=CI_low, xmax=CI_high), width=.5) +
  facet_grid(Year~season) +
